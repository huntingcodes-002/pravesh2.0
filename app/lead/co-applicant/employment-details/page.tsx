...
function EmploymentDetailsContent() {
  ...
  const [formData, setFormData] = useState({
    ...
    yearsInProfessionSENP: coApplicant?.data?.step5?.yearsInProfessionSENP || '',
    monthsInProfessionSENP: coApplicant?.data?.step5?.monthsInProfessionSENP || '',
    ...
    yearsInProfessionSEP: coApplicant?.data?.step5?.yearsInProfessionSEP || '',
    monthsInProfessionSEP: coApplicant?.data?.step5?.monthsInProfessionSEP || '',
  ...
  const handleSave = () => {
    if (!currentLead || !coApplicantId || !coApplicant) {
      toast({
        title: 'Error',
        description: 'Co-applicant not found. Please try again.',
        variant: 'destructive',
      });
      return;
    }
      ...
      >
        <div className="max-w-2xl mx-auto pb-24">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <p className="text-center text-gray-500">Co-applicant not found.</p>
          </div>
        </div>
      </DashboardLayout>
    );
...
export default function CoApplicantEmploymentDetailsPage() {
  return (
    <Suspense fallback={
      <DashboardLayout
        title="Employment Details"
