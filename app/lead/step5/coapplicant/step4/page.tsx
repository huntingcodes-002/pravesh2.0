...
function CoApplicantStep4PageContent() {
  ...
  const coApplicantData = coApplicant?.data?.step4 || {};
  ...
  const [formData, setFormData] = useState({
    ...
    // Others
    natureOfOccupation: coApplicantData.natureOfOccupation || '',
    // Salaried
    employerName: coApplicantData.employerName || '',
    natureOfBusiness: coApplicantData.natureOfBusiness || '',
    industry: coApplicantData.industry || '',
    employmentStatus: coApplicantData.employmentStatus || '',
    employedFrom: coApplicantData.employedFrom || '',
    employedTo: coApplicantData.employedTo || '',
    officialEmail: coApplicantData.officialEmail || '',
    // Self Employed Non-Professional
    orgNameSENP: coApplicantData.orgNameSENP || '',
    natureOfBusinessSENP: coApplicantData.natureOfBusinessSENP || '',
    industrySENP: coApplicantData.industrySENP || '',
    yearsInProfessionSENP: coApplicantData.yearsInProfessionSENP || '',
    monthsInProfessionSENP: coApplicantData.monthsInProfessionSENP || '',
    officialEmailSENP: coApplicantData.officialEmailSENP || '',
    // Self Employed Professional
    orgNameSEP: coApplicantData.orgNameSEP || '',
    natureOfProfession: coApplicantData.natureOfProfession || '',
    industrySEP: coApplicantData.industrySEP || '',
    registrationNumber: coApplicantData.registrationNumber || '',
    yearsInProfessionSEP: coApplicantData.yearsInProfessionSEP || '',
    monthsInProfessionSEP: coApplicantData.monthsInProfessionSEP || '',
    officialEmailSEP: coApplicantData.officialEmailSEP || '',
  });
  ...
  const handleNext = () => {
    ...
    updateCoApplicant(currentLead.id, coApplicantId, {
        currentStep: 4,
        isComplete: true, // Mark as complete here
        data: {
            ...coApplicant?.data,
            step4: formData,
        }
    ...
    // Go back to the main Step 5 (Rule c)
    router.push('/lead/step5');
  };

  const handlePrevious = () => {
    // Previous should go to co-applicant step 3
    if (coApplicantId) {
      router.push(`/lead/step5/coapplicant/step3?coApplicantId=${coApplicantId}`);
    }
  };

  const canProceedCheck = () => {
    ...
    >
      <div className="max-w-2xl mx-auto pb-24">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">Co-Applicant Employment Information</h2>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-4">
            <div className="flex gap-3 max-w-2xl mx-auto">
                ...
                <Button onClick={handleNext} disabled={!canProceedCheck()} className="flex-1 h-12 rounded-lg bg-[#0072CE] hover:bg-[#005a9e]">
...
export default function CoApplicantStep4Page() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center">Loading...</div>}>
      <CoApplicantStep4PageContent />
    </Suspense>
  );
}